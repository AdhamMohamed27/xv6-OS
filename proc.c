...
//   if (!stack || (uint)stack % PGSIZE != 0) {  // Check for valid page-aligned stack
//       return -1;
//   }


//   np = allocproc();
//   if (!np) {  // Fix incorrect check
//       return -1;
//   }


//   np->pgdir = copyuvm(curproc->pgdir, curproc->sz); // Duplicate memory space
//   if (!np->pgdir) {
//       kfree(np->kstack);
//       np->state = UNUSED;
//       return -1;
...
//   np->tf->esp = (uint)stack + PGSIZE - 4;  // Set up stack
//   *(uint *)(np->tf->esp) = (uint)arg; // Push argument
//   np->tf->esp -= 4;
//   *(uint *)(np->tf->esp) = 0; // Fake return address
//   np->tf->eip = (uint)fn;  // Set new instruction pointer


//   for (int i = 0; i < NOFILE; i++) { // Copy file descriptors
//       if (curproc->ofile[i]) {
//           np->ofile[i] = filedup(curproc->ofile[i]);
//       }
//   }
...
//   np->state = RUNNABLE;  // **Fix missing process state**
//   return np->pid;
// }
